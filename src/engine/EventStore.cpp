...
qint64 EventStore::appendEvent(const QString& kind, int epoch, qint64 tsMs, const QString& payload)
{
    if (!m_open)
        return -1;
    QSqlDatabase db = QSqlDatabase::database(m_connectionName);
    QSqlQuery q(db);
    q.prepare(QStringLiteral(
        "INSERT INTO events (epoch, ts, kind, payload) VALUES (?, ?, ?, ?)"));
...
QVector<Event> EventStore::events() const
{
    ...
    if (!q.exec(QStringLiteral(
            "SELECT seq, epoch, ts, kind, payload FROM events ORDER BY seq"))) {
    ...
    while (q.next()) {
        ...
        e.epoch   = q.value(1).toInt();
        e.tsMs    = q.value(2).toLongLong();
        e.kind    = q.value(3).toString();
        e.payload = q.value(4).toString();
        out.append(e);
    }
    return out;
}
